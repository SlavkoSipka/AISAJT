  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <div className="portal-card portal-animate-in">
        <p className="text-[11px] text-[#9aa3b2] mb-1">Napredak projekta</p>
        <p className="text-[22px] font-semibold text-[#1a2030] tracking-[-0.5px] mb-2">
      ...
      <div className="portal-card portal-animate-in" style={{ animationDelay: '0.05s' }}>
        <p className="text-[11px] text-[#9aa3b2] mb-1">Procenjeno završavanje</p>
        {estimatedCompletion ? (
          <>
            <p className="text-[22px] font-semibold text-[#1a2030] tracking-[-0.5px]">
              {new Date(estimatedCompletion).toLocaleDateString('sr-Latn', { day: 'numeric', month: 'short' })}
            </p>
            <div className="flex items-center gap-1.5 mt-1">
              {near ? (
                <>
                  <AlertTriangle size={12} className="text-[#e8970a]" />
                  <span className="text-[12px] text-[#e8970a]">Približava se rok</span>
                </>
              ) : (
                <>
                  <Calendar size={12} className="text-[#9aa3b2]" />
          ...
          <p className="text-[14px] text-[#9aa3b2] mt-1">Nije definisano</p>
        )}
      </div>
      ...
      <div className="portal-card portal-animate-in flex flex-col justify-center" style={{ animationDelay: '0.1s' }}>
        <p className="text-[11px] text-[#9aa3b2] mb-1">Projekat</p>
        <p className="text-[16px] font-semibold text-[#1a2030] tracking-[-0.3px] truncate">{project.name}</p>
        {project.domain && (
          <p className="text-[12px] text-[#9aa3b2] mt-0.5 truncate">{project.domain}</p>
        )}
      </div>
    </div>
  );
