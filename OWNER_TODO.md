...
- [ ] **`www.aisajt.com` / `aisajt.com` — wrong domain redirects the other way, needs a Vercel dashboard fix, not code.** Full history: a Screaming Frog crawl originally found `www.aisajt.com` serving full live content with no redirect to the apex domain (only `<link rel="canonical">` pointed at `aisajt.com`, which search engines can ignore). A `www`→apex redirect rule was added to `vercel.json` to fix it. Deployed, and it caused an **infinite redirect loop**: `curl -I` showed `www.aisajt.com` → 308 → `aisajt.com` → 307 → `www.aisajt.com` → ... The apex domain already has a **platform-level redirect back to `www`** configured in Vercel's dashboard (Project → Settings → Domains) — outside this repo, invisible from the code. The two redirects fought each other and broke every uncached asset on the live site. The `vercel.json` rule was reverted the same day to stop the loop; **`aisajt.com` currently 307-redirects to `www.aisajt.com`**, the opposite of what `sitemap.xml` and every canonical tag assume (they all use the bare `aisajt.com` form, per `src/lib/site-config.ts`).
  **The correct fix has to happen in the Vercel dashboard, not in code**: Project → Settings → Domains → set `aisajt.com` as the primary domain with no outbound redirect, and point `www.aisajt.com` to redirect to it instead (this makes it a permanent 308, not the current temporary 307). Do **not** re-add a `www`→apex rule to `vercel.json` while the dashboard still redirects apex→www — that's exactly what caused the loop last time. Verify after the dashboard change with `curl -sIL https://www.aisajt.com/ | grep -iE "^HTTP/|^location:"` — expect a single `308` hop to `https://aisajt.com/`, not a `307` or a loop.
- [ ] **Real Vercel preview deploy**, checking specifically:
  - Does `www.aisajt.com/*` actually redirect to `https://aisajt.com/*` now (the fix above, done in the dashboard)? Re-crawl with Screaming Frog (or `curl -I https://www.aisajt.com/`) after the dashboard change to confirm the "Non-Indexable Canonicals" report comes back clean.
