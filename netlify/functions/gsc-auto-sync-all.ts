...
async function syncProject(projectId: string, property: string, token: string): Promise<string> {
  ...
  // ── 1. Monthly metrics (last full calendar month) ─────────────────────────
  ...
  const monthKey   = startMonth; // YYYY-MM-01
  ...
  // ── 3. Daily data for chart (last 16 months) ──────────────────────────────
  const endDaily   = new Date(now);
  endDaily.setDate(endDaily.getDate() - 3);           // GSC lag ~3 days
  const startDaily = new Date(endDaily);
  startDaily.setDate(startDaily.getDate() - 479);     // ~16 months back
...
export default async (req: Request, _ctx: Context) => {
  ...
  try {
    ...
    // Fetch all projects with auto-update enabled
    ...
    for (const project of projects) {
      try {
        ...
        results.push({ project_id: project.id, status: 'error', detail: e.message });
      }
    }
